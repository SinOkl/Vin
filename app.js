...
const VINTYPER = ['Rødvin', 'Hvitvin', 'Rosévin', 'Musserende', 'Dessertvin/Portvin', 'Annet'];
...
const AI_PROMPT_MAL = `Du er en vinekspert. Jeg skal registrere en vin i vinkatalogen min. Se på bildet/beskrivelsen av vinen jeg gir deg, og svar KUN med gyldig JSON (ingen forklaringstekst, ingen kodeblokk-merking rundt) i nøyaktig dette formatet:

{
  "navn": "",
  "produsent": "",
  "argang": "",
  "type": "Rødvin | Hvitvin | Rosévin | Musserende | Dessertvin/Portvin | Annet",
  "land": "",
  "region": "",
  "druer": "",
  "drikkeklarFra": "",
...
- "type" må være nøyaktig én av verdiene i listen.
- "matparKategorier" må kun inneholde verdier fra listen over, som en JSON-liste.
- "lagringstemperatur" er hvor kaldt vinen bør oppbevares over tid, "serveringstemperatur" er hvor kald den bør være når den drikkes — disse er ofte ulike, ikke forveksle dem.
- Skriv "" for felt du er usikker på — ikke gjett blindt.
- drikkeklarFra/drikkeklarTil er årstall (f.eks. 2026), et estimat basert på vintype og årgang.
- Skal du registrere flere viner samtidig: svar med en JSON-liste av slike objekter i stedet for ett enkelt objekt.

Her er vinen: [lim inn bilde av etiketten, eller beskriv vinen (navn, produsent, årgang) her]`;
...
// Gjør om et løst/upresist objekt (typisk fra en AI) til en gyldig vin. Returnerer null hvis navn mangler.
function normaliserImportertVin(raw) {
  ...
  let type = raw.type;
  if (!VINTYPER.includes(type)) type = 'Annet';
...
// Tar imot rå JSON-tekst (ett vinobjekt eller en liste), validerer og lagrer gyldige viner.
async function importerFraJsonTekst(tekst) {
  ...
  if (!gyldige.length) {
    throw new Error('Fant ingen gyldige viner i JSON-en (mangler "navn"-felt?).');
  }
...
function visOversikt() {
  ...
  const unikeViner = iKjelleren.length;
  ...
  app.appendChild(el(`
    <div class="side">
      <h1>🍷 Vinkjelleren</h1>
      <div class="statgrid">
        <div class="statkort"><span class="stattall">${unikeViner}</span><span class="statlabel">viner</span></div>
      ...
      <section>
        ...
        ${klarNa.length ? listeKompakt(klarNa.slice(0, 8)) : '<p class="tom">Ingen viner er markert som klare akkurat nå.</p>'}
      </section>

      ${unikeViner === 0 ? `<p class="tom">Vinkjelleren er tom. Trykk «Legg til» for å registrere din første vin.</p>` : ''}

      ${drukket.length ? `<p class="hjelpetekst"><a href="#/viner">🍾 ${drukket.length} vin(er) i drikkehistorikken →</a></p>` : ''}
    </div>
  `));
}
...
let filterState = { sok: '', type: '', matpar: '', status: '', visning: 'kjeller' };

function visVinliste() {
  app.innerHTML = '';
  const wrap = el(`
    <div class="side">
      <h1>Viner</h1>
      <div class="visning-bytter">
        <button type="button" class="visning-knapp ${filterState.visning === 'kjeller' ? 'aktiv' : ''}" id="knapp-kjeller">🍷 I kjelleren</button>
      ...
      <div class="filterrad">
        ...
        <div class="filterrad2">
          <select id="filter-type">
            ...
            ${VINTYPER.map((t) => `<option value="${t}" ${filterState.type === t ? 'selected' : ''}>${t}</option>`).join('')}
          </select>
...
function renderVinlisteResultat() {
  ...
  if (!filtrert.length) {
    resultat.innerHTML = `<p class="tom">${filterState.visning === 'drukket' ? 'Ingen viner i drikkehistorikken ennå.' : 'Ingen viner matcher filteret.'}</p>`;
    return;
  }
...
function vinkortHtml(v) {
  ...
  return `
    <a class="vinkort ${v.drukketDato ? 'drukket' : ''}" href="#/vin/${v.id}">
      <div class="vinkort-bilde">${v.bilde ? `<img src="${v.bilde}" alt="">` : '🍷'}</div>
...
async function visDetalj(id) {
  ...
  const status = drikkestatus(v);
  const forslag = LAGRINGSFORSLAG[v.type] || LAGRINGSFORSLAG['Annet'];

  app.innerHTML = '';
  app.appendChild(el(`
    <div class="side">
      <a href="#/viner" class="tilbake">← Tilbake til viner</a>
      ${v.bilde ? `<img class="detaljbilde" src="${v.bilde}" alt="">` : ''}
      <h1>${escapeHtml(v.navn)}${v.argang ? ` <span class="argang">${escapeHtml(v.argang)}</span>` : ''}</h1>
      ${v.drukketDato
        ? `<span class="status-badge status-drukket">🍾 Drukket ${escapeHtml(v.drukketDato)}</span>`
        : `<span class="status-badge ${status.klasse}">${status.label}</span>`}
      ...
      <section class="detaljseksjon">
        ...
        <dl class="detaljliste">
          ...
          ${dRad('Innkjøpspris', formatKr(v.innkjopspris))}
  ...
  document.getElementById('slett-knapp').addEventListener('click', async () => {
    if (confirm(`Slette «${v.navn}» fra vinkjelleren?`)) {
...
// Fyller det synlige vinskjemaet med felter fra et (allerede normalisert) vinobjekt.
function fyllSkjemaFraVin(skjema, data) {
  ...
  const forslag = LAGRINGSFORSLAG[data.type];
  ...
  if (forslag && tempFelt && fuktFelt) {
    tempFelt.placeholder = forslag.temp;
    fuktFelt.placeholder = forslag.fuktighet;
    if (serveFelt) serveFelt.placeholder = forslag.servering;
  }
}

// ---------- Visning: Skjema (legg til / rediger) ----------

async function visSkjema(id) {
  const eksisterende = id ? (alleViner.find((x) => x.id === id) || await VinDB.hent(id)) : null;
  const v = eksisterende || {
    navn: '', produsent: '', argang: '', type: 'Rødvin', land: '', region: '', druer: '',
  ...
  app.appendChild(el(`
    <div class="side">
      <h1>${eksisterende ? 'Rediger vin' : 'Legg til vin'}</h1>
      ...
      <section class="detaljseksjon">
        ...
        <p class="hjelpetekst">
          ...
          sammen med malen under. Lim JSON-svaret inn i feltet — én vin fyller ut
          skjemaet under så du kan sjekke det før lagring, en hel liste med viner
          importeres rett inn.
        </p>
        <div class="knapperad">
        ...
        <label class="importlabel">Lim inn JSON-svar fra AI-en
          <textarea id="ai-json-felt" rows="5" placeholder='{"navn": "...", "type": "Rødvin", ...}'></textarea>
        </label>
        <div class="knapperad">
      ...
      <form id="vinskjema" class="skjema">
        ...
        <div class="to-kolonner">
          ...
          <label>Type
            <select name="type">${VINTYPER.map((t) => `<option ${v.type === t ? 'selected' : ''}>${t}</option>`).join('')}</select>
          </label>
        </div>
        <div class="to-kolonner">
        ...
        <div class="to-kolonner">
          <label>Innkjøpspris (kr)<input type="number" min="0" name="innkjopspris" value="${escapeHtml(v.innkjopspris)}"></label>
        ...
        <div class="to-kolonner">
          <label>Lagringstemperatur<input name="lagringstemperatur" id="temp-felt" placeholder="${LAGRINGSFORSLAG[v.type]?.temp || ''}" value="${escapeHtml(v.lagringstemperatur)}"></label>
          <label>Fuktighet<input name="lagringsfuktighet" id="fukt-felt" placeholder="${LAGRINGSFORSLAG[v.type]?.fuktighet || ''}" value="${escapeHtml(v.lagringsfuktighet)}"></label>
        </div>
        <label>Serveringstemperatur<input name="serveringstemperatur" id="serve-felt" placeholder="${LAGRINGSFORSLAG[v.type]?.servering || ''}" value="${escapeHtml(v.serveringstemperatur)}"></label>
        <div class="to-kolonner">
        ...
        <div class="knapperad">
          ...
          <a class="knapp" href="${eksisterende ? '#/vin/' + v.id : '#/viner'}">Avbryt</a>
        </div>
      </form>
    </div>
  `));
  ...
  skjema.querySelector('select[name="type"]').addEventListener('change', (e) => {
    const forslag = LAGRINGSFORSLAG[e.target.value];
    if (forslag) {
      document.getElementById('temp-felt').placeholder = forslag.temp;
      document.getElementById('fukt-felt').placeholder = forslag.fuktighet;
      document.getElementById('serve-felt').placeholder = forslag.servering;
    }
  });
  ...
  if (aiBrukKnapp) {
    aiBrukKnapp.addEventListener('click', async () => {
      ...
      if (liste.length === 1) {
        ...
        if (!vinData) { alert('Fant ingen gyldig vin i JSON-en (mangler «navn»-felt?).'); return; }
        fyllSkjemaFraVin(skjema, vinData);
        felt.value = '';
      } else {
        try {
          const { antall, hoppetOver } = await importerFraJsonTekst(tekst);
          alert(`Importerte ${antall} vin(er) direkte.${hoppetOver ? ` Hoppet over ${hoppetOver} som manglet navn.` : ''}`);
          location.hash = '#/viner';
        } catch (err) {
...
function visInnstillinger() {
  ...
  document.getElementById('importer-input').addEventListener('change', async (e) => {
    ...
    let melding = `Importerte ${totalAntall} vin(er) fra ${filer.length} fil(er).`;
  ...
  document.getElementById('slett-alt-knapp').addEventListener('click', async () => {
    if (confirm('Sikker på at du vil slette ALLE viner? Dette kan ikke angres.')) {
